...
// ============================================================
//  JOUEURS — modifiez cette liste selon vos besoins
// ============================================================
...
function getPhaseFor(ps) {
  ...
  if (ps.animalWinner) {
    return Object.keys(ps.qualityVotes || {}).length >= voters ? 'quality-runoff' : 'quality-propose';
  }
...
.header-title{font-family:'Cormorant Garamond',serif;font-size:1.35rem;font-weight:700;color:var(--amber);flex:1;text-align:center;}
...
export default function App() {
  ...
  useEffect(() => { setSelCard(null); setSelRunoff(null); }, [gs]);
  ...
  const saveState = useCallback(async (next) => {
    setBusy(true);
    try {
      await set(ref(db, 'gameState'), next);
    } catch (e) {
      showToast('Erreur réseau ⚠️');
    } finally {
      setBusy(false);
    }
  }, [showToast]);

  const resolveRunoffIfReady = (state, receiverName, cardType) => {
    const ps = state[receiverName];
    const voters = PLAYERS.length - 1;
    const runoffVotes = cardType === 'animal' ? ps.animalRunoffVotes : ps.qualityRunoffVotes;
    if (!runoffVotes || Object.keys(runoffVotes).length < voters) return state;
    const counts = {};
    Object.values(runoffVotes).forEach(cid => { counts[cid] = (counts[cid] || 0) + 1; });
    const takenIds = getTakenIds(state, cardType, receiverName);
    ...
    const next = { ...state, [receiverName]: { ...ps } };
    if (cardType === 'animal') next[receiverName].animalWinner = winnerId;
    else next[receiverName].qualityWinner = winnerId;
    return next;
  };

  const submitProposal = async (receiverName, cardType) => {
    if (!selCard || !gs) return;
    const next = JSON.parse(JSON.stringify(gs));
    if (cardType === 'animal') {
      next[receiverName].animalVotes = { ...next[receiverName].animalVotes, [me]: selCard };
    } else {
      next[receiverName].qualityVotes = { ...next[receiverName].qualityVotes, [me]: selCard };
    }
    await saveState(next);
    showToast('Proposition enregistrée ! 🌿');
  };

  const submitRunoff = async (receiverName, cardType) => {
    if (!selRunoff || !gs) return;
    let next = JSON.parse(JSON.stringify(gs));
    if (cardType === 'animal') {
      next[receiverName].animalRunoffVotes = { ...next[receiverName].animalRunoffVotes, [me]: selRunoff };
    } else {
      next[receiverName].qualityRunoffVotes = { ...next[receiverName].qualityRunoffVotes, [me]: selRunoff };
    }
    next = resolveRunoffIfReady(next, receiverName, cardType);
    await saveState(next);
  ...
  const goBack = () => { setMe(null); setView('login'); };
  ...
  const phaseLabel = (name) => {
    ...
    if (ps.animalWinner) {
      ...
      if (qrv >= voters) return '⏳ Résultat qualité…';
      if (qv >= voters) return `${qrv}/${voters} votes final qualité`;
      return `${qv}/${voters} propositions qualité`;
    }
    const av = Object.keys(ps.animalVotes || {}).length;
    const arv = Object.keys(ps.animalRunoffVotes || {}).length;
    if (arv >= voters) return '⏳ Résultat animal…';
    if (av >= voters) return `${arv}/${voters} votes final animal`;
    return `${av}/${voters} propositions animal`;
  };
          ...
          <div className="players-grid">
            {PLAYERS.map(p => (
              <button key={p} className={`player-tile${gs[p]?.qualityWinner ? ' done' : ''}`}
                onClick={() => { setMe(p); setView('game'); }}>
                {p}
                <span className="tile-status">{phaseLabel(p)}</span>
              </button>
            ))}
          </div>
      ...
      <>
        ...
        <div className="screen">
          <div className="game-header">
            <button className="header-back" onClick={goBack}>← Retour</button>
          ...
          <div className="game-body">
            ...
            <div className="bgroup" style={{ justifyContent: 'center', marginTop: '2rem' }}>
  ...
  if (view === 'game' && me && gs[me]) {
    const phase = getPhaseFor(gs[me]);
    const phaseHeaderLabel = {
      'animal-propose': 'Phase 1 · Animal',
      'animal-runoff': 'Vote final · Animal',
      'quality-propose': 'Phase 1 · Qualité',
      'quality-runoff': 'Vote final · Qualité',
      'done': 'Totem complet ✓'
    }[phase] || '';
    ...
    return (
      <>
        ...
        <div className="screen">
          <div className="game-header">
            <button className="header-back" onClick={goBack}>← Accueil</button>
            <div className="header-title">🌿 {me}</div>
            <span className="header-phase">{phaseHeaderLabel}</span>
            <div className={`sync-dot${busy ? ' busy' : ''}`} />
          </div>
          <div className="game-body">
            {phase === 'done' && <FinalTotem gs={gs} name={me} goBack={goBack} />}
            {phase === 'animal-propose' && <ProposeView gs={gs} receiver={me} me={me} cardType="animal" selCard={selCard} setSelCard={setSelCard} submitProposal={submitProposal} />}
            {phase === 'animal-runoff' && <RunoffView gs={gs} receiver={me} me={me} cardType="animal" selRunoff={selRunoff} setSelRunoff={setSelRunoff} submitRunoff={submitRunoff} />}
            {phase === 'quality-propose' && <ProposeView gs={gs} receiver={me} me={me} cardType="quality" selCard={selCard} setSelCard={setSelCard} submitProposal={submitProposal} />}
            {phase === 'quality-runoff' && <RunoffView gs={gs} receiver={me} me={me} cardType="quality" selRunoff={selRunoff} setSelRunoff={setSelRunoff} submitRunoff={submitRunoff} />}
          </div>
        </div>
        <Toast t={toast} />
      </>
...
function ProposeView({ gs, receiver, me, cardType, selCard, setSelCard, submitProposal }) {
  ...
  const voted = Object.keys(safeVotes);
  ...
  if (me === receiver) return (
    <>
      ...
      <div className="receiver-banner">
        <h2>✨ {receiver} ✨</h2>
        <p>Vos amies choisissent {isAnimal ? 'votre animal totem' : 'votre qualité totem'} en secret</p>
      </div>
      ...
      <div className="wait"><div className="wi">🌸</div><h3>Installez-vous !</h3><p>Vos amies choisissent en secret {isAnimal ? 'un animal' : 'une qualité'} pour vous.</p></div>
    </>
  );

  const myVote = safeVotes[me];
  if (myVote) {
    ...
    return (
      <>
        ...
        <div className="wait"><div className="wi">⏳</div><h3>Proposition enregistrée !</h3><p>Attendez le vote final.</p></div>
      </>
    );
  }
  ...
  return (
    <>
      ...
      <div className="spanel"><strong>{voted.length}/{others.length}</strong> joueuses ont proposé<div className="pbar"><div className="pfill" style={{ width: `${voted.length / others.length * 100}%` }} /></div></div>
      ...
      <div className="bgroup">
        <button className="btn btn-p" disabled={!selCard} onClick={() => submitProposal(receiver, cardType)}>Proposer cette carte</button>
      </div>
    </>
  );
}
...
function RunoffView({ gs, receiver, me, cardType, selRunoff, setSelRunoff, submitRunoff }) {
  ...
  if (me === receiver) return (
    <>
      ...
      <div className="receiver-banner"><h2>🗳️ Vote final pour {receiver}</h2><p>Vos amies votent pour votre {isAnimal ? 'animal' : 'qualité'} totem !</p></div>
      <div className="spanel">
        <strong>{voted.length}/{others.length}</strong> votes enregistrés
        <div className="pbar"><div className="pfill" style={{ width: `${voted.length / others.length * 100}%` }} /></div>
        {remaining.length > 0 ? <span>En attente : {remaining.join(', ')}</span> : <span className="gold">✨ Vote terminé ! Votre totem va être révélé.</span>}
      </div>
      <div className="wait"><div className="wi">🎊</div><h3>Le vote final est en cours !</h3><p>Vos amies choisissent votre {isAnimal ? 'animal' : 'qualité'} totem définitif.</p></div>
    </>
  );
  ...
  return (
    <>
      ...
      <div className="bgroup">
        <button className="btn btn-p" disabled={!selRunoff} onClick={() => submitRunoff(receiver, cardType)}>Voter pour cette carte</button>
      </div>
    </>
  );
}
  ...
  return (
    <>
      ...
      <div className="bgroup" style={{ justifyContent: 'center' }}>
        <button className="btn btn-g" onClick={goBack}>← Retour à l'accueil</button>
      </div>
    </>
  );
}
